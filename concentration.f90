...
program concentration
    ...
    logical :: repeat_flag
    character(len=30) :: data_string
    type(bolsig_data_t) :: big_data

    big_data = read_bolsig_file("output.dat")
    call print_bolsig_info(big_data)
!
    ch_nt0 = 'Data_\'
    folder_name = ch_nt0 // trim(data_string())
...
!fav   k_i is for normal conditions. k_i in cm^2/(V s) = 1.0e-4 m^2/(V s)
    ...
    do k = 0, M-1
        n_e_i(k) = ne0 * bessel_jn(0, r_k(k) * 2.40482_dp / r0)
        !print *, n_e_i(k), r_k(k) * 2.40483_dp / r0
    end do
    n_e_i(M) = 0
    n_i_i = n_e_i
...
!    end if
    ...
    tau = minval(h_k)**2 /(4.0_dp*max(D_e, D_i))
    print *, tau
