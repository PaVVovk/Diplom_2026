...
! Выделение памяти
    ...
    N = p/(k_b*T_gas)
    EN = big_data%E_over_N(4)
    D_e = big_data%diffusion_N(4)/N
    k_e = big_data%mobility_N(4)/N
    k_ion = big_data%ionization_freq_N(4)
...
!       r_half_k(M) = r_k(M)    !fav
    do k = 1, M-1
        ...
        a_km(k) = 2*(r_half_k(k-1)/(r_half_k(k-1) + r_half_k(k+1)))/h_half_k(k)
        b_km(k) = 2*(r_half_k(k)/(r_half_k(k-1) + r_half_k(k+1)))/h_half_k(k)!(r_half_k(k+1)/r_k(k))/h_half_k(k)
